			...
			@endif
				...
				<ul class="navbar-nav ml-auto">
					...
					<li class="nav-item mr-1">
						<a @if (auth()->guest() && request()->route()->named('profile')) data-toggle="modal" data-target="#loginFormModal" @endif class="nav-link login-btn @if ($settings->registration_active == '0')  btn btn-main btn-primary pr-3 pl-3 @endif" href="{{$settings->home_style == 0 ? url('login') : url('/')}}">
							{{trans('auth.login')}}
						</a>
					</li>
					...
					<li class="nav-item">
						<a @if (auth()->guest() && request()->route()->named('profile')) id="toggleRegister" data-toggle="modal" data-target="#loginFormModal" @endif class="nav-link btn btn-main btn-primary pr-3 pl-3" href="{{$settings->home_style == 0 ? url('signup') : url('/')}}">
