...
class UserController extends Controller
{
      ...
      ->mySubscriptions()
        ->where('stripe_id', '=', '')
          ...
          ->orWhere('stripe_status', 'active')
            ->where('stripe_id', '<>', '')
              ->whereStripePrice(auth()->user()->plan)
              ->orWhere('stripe_id', '=', '')
  ...
  public function postDetail($slug, $id)
  {
    ...
    $updateCount = $updates->count();
      ...
      if (auth()->check() && $updateCount != 0 && $updates[0]->user_id != auth()->user()->id
        && $updates[0]->status == 'pending'
        && auth()->user()->role != 'admin'
      ) {
  			abort(404);
  		} else if (auth()->guest() && $updateCount != 0 && $updates[0]->status == 'pending') {
  			abort(404);
  		}
  ...
  if (auth()->user()->verified_id == 'no' || auth()->user()->verified_id == 'reject') {
    ...
    $this->settings->min_subscription_amount = $this->settings->min_subscription_amount;
     ...
     if ($validator->fails()) {
      ...
      $user->free_subscription = $freeSubscription;
      $user->plan              = 'user_'.auth()->user()->id;
      $user->save();
  ...
  protected function createPlanPaystack()
  {
   ...
   $path = config('path.cover');
    ...
    public function invoiceDeposits($id)
    {
      ...
      $totalAmount = ($data->amount + $data->transaction_fee);
    ...
    public function myPosts()
    {
      ...
      $posts = Updates::whereUserId(auth()->user()->id)
      ->where('status', '<>', 'encode')
      ->orderBy('id', 'desc')
      ->paginate(20);
    ...
    {
      $transactions = ReferralTransactions::whereReferredBy(auth()->user()->id)
      ->orderBy('id', 'desc')
      ->paginate(20);
